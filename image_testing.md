...
- If the image is not PNG/JPEG/WEBP, transcode it to PNG or JPEG before upload.
  - Example: if you read a `.jpg` but the content is actually PNG after conversion — re-detect and update the MIME.
- If the image is animated (e.g., GIF, APNG, animated WEBP), extract the first frame only.
